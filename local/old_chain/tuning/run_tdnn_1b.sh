...
if [ $stage -le 12 ]; then
  ...
  if [ -d $lang ]; then
    if [ $lang/L.fst -nt data/speechdat_lang/L.fst ]; then
...
if [ $stage -le 13 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 100 --cmd "$train_cmd" ${lores_train_data_dir} \
    data/speechdat_lang $gmm_dir $lat_dir
...
if [ $stage -le 17 ]; then
  ...
  utils/lang/check_phones_compatible.sh \
    data/kielipankki_lang/phones.txt $lang/phones.txt
  utils/mkgraph.sh \
    --remove-oov --self-loop-scale 1.0 data/kielipankki_lang \
    $tree_dir $tree_dir/graph_chain_kielipankki || exit 1;
fi
...
if [ $stage -le 18 ]; then
  ...
  for data in $test_sets; do
    (
      ...
      for lmtype in chain_kielipankki; do
